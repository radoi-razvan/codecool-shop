    ...
    cartItemBuilder: function (product) {
        return `<div id="cartitem${product.Id}" class="Cart-Items-cart">
                <div class="image-box-cart">
                    <img src="https://localhost:44368/img/${product.Name}.jpg" class="cart-img" />
                </div>
                <div class="about-cart centered">
                    <h1 class="title-cart text-md-center">${product.Name}</h1>
                </div>
                <div class="counter-cart">
                        <i class="bi bi-plus-circle text-dark product-increase" data-increase="increase-${product.Id}" style="font-size: 20px"></i>
                    <div class="count-cart">${product.CartProduct.Quantity}</div>
                        <i class="bi bi-dash-circle text-dark product-remove" data-remove="remove-${product.Id}" style="font-size: 20px"></i>
                </div>
                <div class="prices-cart">
                    <div class="amount-cart">$${parseFloat(product.DefaultPrice).toFixed(2)}</div>
                    <div class="remove-cart">
                            <i class="bi bi-x-circle text-danger product-delete"  data-delete="delete-${product.Id}"></i>
                    </div>
                </div>
            </div>
            <hr class="bg-dark" />`;
        ...
        return `<div id="product${product.Id}" class="col-lg-3 col-lg-3 card-container" style="max-width: 350px;">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <a type="button" class="btn btn-warning add-cart-btn" data-product="dataProduct-${product.Id}" style="font-weight: bold">
